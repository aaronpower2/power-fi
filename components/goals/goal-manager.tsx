...
function normalizeGoalPercentValues<
  T extends {
...
>(values: T): T {
  return {
    ...
    withdrawalRatePercent:
      roundToSingleDecimal(values.withdrawalRatePercent) ??
      values.withdrawalRatePercent,
    targetSavingsRatePercent:
      values.targetSavingsRatePercent == null
        ? null
        : (roundToSingleDecimal(values.targetSavingsRatePercent) ??
          values.targetSavingsRatePercent),
  }
}
...
function CopyBudgetCategoriesIntoLifestyle({
    ...
    return categories.map((c) => {
      ...
      if (native <= 0) {
        ...
        if (!rateMap) {
          amountInGoal = null
          reasonDisabled =
            "FX rates unavailable — cannot convert to goal currency"
        } else {
      ...
      const selectable =
        reasonDisabled == null && amountInGoal != null && amountInGoal >= 0.01
  ...
  const selectableIds = useMemo(
    () => rows.filter((r) => r.selectable).map((r) => r.id),
    [rows]
  )
    ...
    () =>
      categories.some(
        (c) =>
          ...
          c.nativeCurrency.trim().toUpperCase() !==
            goalCurrency.trim().toUpperCase()
      ),
  ...
  const selectedCount = rows.filter(
    (r) => r.selectable && selected.has(r.id)
  ).length
      ...
      >
        <div className="shrink-0 border-b border-border px-4 py-3">
          ...
          <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
            Uses each category&apos;s smoothed monthly plan (same as Cash Flow).
            New lines are appended; edit or remove afterward. Goal currency:{" "}
            <span className="font-medium text-foreground">{goalCurrency}</span>
            {fxAsOfDate ? (
              <span className="mt-1 block">
                FX as of {fxAsOfDate} when converting.
              </span>
            ) : needsFx ? (
              <span className="mt-1 block text-destructive">
                No FX snapshot — run{" "}
                <code className="text-xs">pnpm fx:sync</code> to convert other
                currencies.
              </span>
            ) : null}
          </p>
        ...
        <div className="max-h-[min(20rem,50vh)] overflow-y-auto px-4 py-2">
          {categories.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              No expense categories yet. Add them under Cash Flow, then set
              recurring plans on categories you want here.
            </p>
          ) : selectableIds.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              No categories have a convertible monthly plan for {goalCurrency}.
              Set recurring category budgets under Cash Flow first.
            </p>
          ) : (
            <div className="space-y-3">
              ...
              <div className="space-y-0.5">
                <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] gap-x-3 text-xs font-medium text-muted-foreground">
                    ...
                    >
                      ...
                      <div className="min-w-0 space-y-0.5">
                        ...
                        {!r.selectable && r.reasonDisabled ? (
                          <p className="text-xs leading-snug wrap-break-word text-muted-foreground">
                            {r.reasonDisabled}
                          </p>
                        ) : r.nativeCurrency.trim().toUpperCase() !==
                          goalCurrency.trim().toUpperCase() ? (
                          <p className="text-xs leading-snug wrap-break-word text-muted-foreground tabular-nums">
                            {formatCurrency(
                              r.monthlyAmountNative,
                              r.nativeCurrency
                            )}{" "}
                            planned
                          </p>
                      ...
                      <p className="shrink-0 self-center text-end text-sm whitespace-nowrap text-foreground tabular-nums">
                        {r.amountInGoal != null
                          ? formatCurrency(r.amountInGoal, goalCurrency)
                          : "—"}
                      </p>
                    </li>
                  ))}
...
function LifestyleLinesFields({
  ...
  const total =
    lines?.reduce((s: number, l: { monthlyAmount?: unknown }) => {
      const n =
        typeof l?.monthlyAmount === "number"
          ? l.monthlyAmount
          : Number(l?.monthlyAmount)
    ...
    <div className="space-y-3">
      <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-1">
          ...
          <InfoTooltip>
            Name each part of your target lifestyle. Amounts are monthly costs
            in your goal currency ({currencyCode}).
          </InfoTooltip>
        </div>
...
export function GoalManager({
  ...
  const activeGoal =
    items.find(
      (item) => item.goal.id === planningData.summary.reportingGoalId
    ) ?? null
            ...
            <div className="min-w-0 flex-1 text-sm text-muted-foreground">
              {activeGoal ? (
                ...
                <p className="leading-snug">
                  No active goal yet — mark one below to power FI Summary and
                  projections.
                </p>
              )}
            </div>
              ...
              >
                {planningData.summaryCurrencyOptions.map((code) => (
                  <Button
                    ...
                    size="sm"
                    variant={
                      planningData.summary.reportingCurrency === code
                        ? "secondary"
                        : "ghost"
                    }
                    ...
                    onClick={() => {
                      router.push(
                        `${dashboardRoutes.goal}?ccy=${encodeURIComponent(code)}`
                      )
                    }}
                  >
                    {code}
          ...
          <CardHeaderTitleRow
            ...
            info={
              <>
                ...
                used for FI summary and projections. Required portfolio uses:
                (sum of lifestyle lines × 12) ÷ withdrawal rate. Withdrawal rate
...
function defaultLifestyleLines(
  ...
  return [
    {
      name: "Lifestyle",
      monthlyAmount: Number(goal.monthlyFundingRequirement),
    },
  ]
}
...
function GoalEditForm({
  ...
  const form = useForm<UpdateGoalInput>({
    ...
    defaultValues: {
      ...
      targetSavingsRatePercent:
        goal.targetSavingsRate != null
          ? Number(goal.targetSavingsRate) * 100
          : null,
          ...
          render={({ field }) => (
            <FormItem>
              <div className="flex items-center gap-1">
                ...
                <InfoTooltip>
                  Shown in FI summary and goal lists. Use something you’ll
                  recognize (e.g. Lean FIRE, NZ base).
                </InfoTooltip>
              </div>
              <FormControl>
                <Input
                  placeholder="e.g. Coast FI — NZ"
                  autoComplete="off"
                  {...field}
                />
              </FormControl>
          ...
          render={({ field }) => (
            <FormItem>
              <div className="flex items-center gap-1">
                ...
                <InfoTooltip>
                  Lifestyle amounts and FI summary use this currency.
                </InfoTooltip>
              </div>
          ...
          render={({ field }) => (
            <FormItem>
              <div className="flex items-center gap-1">
                ...
                <InfoTooltip>
                  Annual rate; enter 4 for the 4% rule (stored as a decimal
                  internally).
                </InfoTooltip>
              </div>
              <FormControl>
          ...
          render={({ field }) => (
            <FormItem>
              <div className="flex items-center gap-1">
                ...
                <InfoTooltip>
                  Optional benchmark for the Summary dashboard savings-rate
                  card. Leave blank to use the default 40%.
                </InfoTooltip>
              </div>
              <FormControl>
                <PercentInput
      ...
      >
        <DialogHeader className="flex shrink-0 flex-row flex-wrap items-center justify-between gap-2 space-y-0 border-b border-border px-6 py-4 pr-14">
          <div className="flex min-w-0 flex-1 items-center gap-1.5">
            ...
            <InfoTooltip>
              Updates this plan only. The row marked{" "}
              <span className="font-medium text-foreground">Active</span> in the
              table is what FI summary uses.
            </InfoTooltip>
          </div>
          <Button
            type="submit"
...
function CreateGoalDialog({
  ...
  const form = useForm<GoalInput>({
    resolver: zodResolver(goalInputSchema) as Resolver<GoalInput>,
    defaultValues: {
      name: "",
      currency: "USD",
      fiDate: "",
  ...
  async function onSubmit(values: GoalInput) {
    const payload: CreateGoalInput = {
      ...normalizeGoalPercentValues(values),
      makeActive,
    }
      ...
      >
        <DialogHeader className="flex shrink-0 flex-row flex-wrap items-center justify-between gap-2 space-y-0 border-b border-border px-6 py-4 pr-14">
          <div className="flex min-w-0 flex-1 items-center gap-1.5">
            ...
            <InfoTooltip>
              Set FI date, withdrawal rate, and lifestyle lines that sum to your
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-4">
          <Form {...form}>
            ...
            >
                ...
                render={({ field }) => (
                  <FormItem>
                    <div className="flex items-center gap-1">
                      <FormLabel>Goal name</FormLabel>
                      <InfoTooltip>
                        How this goal appears in FI summary and lists.
                      </InfoTooltip>
                    </div>
                    <FormControl>
                      <Input
                        placeholder="e.g. Baseline plan"
                        autoComplete="off"
                        {...field}
                      />
                    </FormControl>
                ...
                render={({ field }) => (
                  <FormItem>
                    <div className="flex items-center gap-1">
                      <FormLabel>Target savings rate (%)</FormLabel>
                      <InfoTooltip>
                        Optional benchmark for the Summary dashboard
                        savings-rate card. Leave blank to use the default 40%.
                      </InfoTooltip>
                    </div>
                    <FormControl>
                      <PercentInput
              ...
              <LifestyleLinesFields
                control={
                  form.control as unknown as Control<GoalLifestyleFormValues>
                }
